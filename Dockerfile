...
#
# Single image that serves every network transport of the `yara` VGI worker:
...
#
# This worker is STATELESS: the YARA-X engine and its modules are compiled into
# the binary and rules are supplied per query (no per-attach state, no model
# registry), so there is no /data volume and no `farm.query.vgi.volumes`
...
# runtime ships. Everything (vgi + yara-x) resolves from crates.io per the
# committed Cargo.lock — no git/patch dependencies.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin yara-worker \
    && cp target/release/yara-worker /yara-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-yara
...
LABEL org.opencontainers.image.title="vgi-yara" \
      org.opencontainers.image.description="Defensive YARA-X malware scanning (compile rules + scan blobs/text) as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_YARA_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /yara-worker /usr/local/bin/yara-worker
